...
export async function analyzeFunction(sourceCode, functionName) {
  ...
  // Escape function name to prevent ReDoS
  const escapedName = escapeRegExp(functionName);
  
  const functionRegex = new RegExp(
    `(?:function\\s+${escapedName}|const\\s+${escapedName}\\s*=|${escapedName}\\s*:\\s*function|${escapedName}\\s*=\\s*\\(|${escapedName}\\s*=\\s*async\\s*\\()([\\s\\S]*?)(?=\\n(?:function|const|export|$))`,
    'i'
  );
